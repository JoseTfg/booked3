...
class Ldap extends Authentication implements IAuthentication
{
	...
	public function aa($username, $password)
	{
		...
		if ($isValid)
		{
			//$this->ldap->user = new LdapUser($currentResult, $this->ldap->options->AttributeMapping());
	...
	public function bb($username, $loginContext)
	{
		$username = $this->CleanUsername($username);
	...
	public function Login($username, $loginContext)
	{
		...
		$registration->Synchronize(
				new AuthenticatedUser(
						$username,$email,"FirstName","LastName",$this->password,Configuration::Instance()->GetKey(ConfigKeys::LANGUAGE),
						Configuration::Instance()->GetDefaultTimezone(),"phone","institution","title"));
		$repo = new UserRepository();
		$user = $repo->LoadByUsername($username);
		$user->Deactivate();
		$user->Activate();
