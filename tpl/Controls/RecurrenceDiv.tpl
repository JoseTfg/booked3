...
<div id="repeatDiv" style="font-size:12px;">
	...
	<div id="{$prefix}repeatEveryDiv" style="display:none;" class="days weeks months years">
		...
		<input type="hidden" id="{$prefix}formattedEndRepeat" {formname key=end_repeat_date} value="{formatdate date=$RepeatTerminationDate key=system}"/>
	</div>
	<div id="{$prefix}repeatOnWeeklyDiv" style="display:none;" class="weeks">
		<label>{translate key="RepeatDaysPrompt"}</label>
		<input type="checkbox" id="{$prefix}repeatDay0" {formname key=repeat_sunday} /><label for="repeatDay0">{translate key="DaySundayAbbr"}</label>
		<input type="checkbox" id="{$prefix}repeatDay1" {formname key=repeat_monday} /><label for="repeatDay1">{translate key="DayMondayAbbr"}</label>
		<input type="checkbox" id="{$prefix}repeatDay2" {formname key=repeat_tuesday} /><label for="repeatDay2">{translate key="DayTuesdayAbbr"}</label>
		<input type="checkbox" id="{$prefix}repeatDay3" {formname key=repeat_wednesday} /><label for="repeatDay3">{translate key="DayWednesdayAbbr"}</label>
		<input type="checkbox" id="{$prefix}repeatDay4" {formname key=repeat_thursday} /><label for="repeatDay4">{translate key="DayThursdayAbbr"}</label>
		<input type="checkbox" id="{$prefix}repeatDay5" {formname key=repeat_friday} /><label for="repeatDay5">{translate key="DayFridayAbbr"}</label>
		<input type="checkbox" id="{$prefix}repeatDay6" {formname key=repeat_saturday} /><label for="repeatDay6">{translate key="DaySaturdayAbbr"}</label>
	</div>

	<div id="repeatOnMonthlyDiv" style="display:none;" class="months">
		<input type="radio" {formname key=REPEAT_MONTHLY_TYPE} value="{RepeatMonthlyType::DayOfMonth}" id="{$prefix}repeatMonthDay" checked="checked"/>
		<label for="repeatMonthDay">{translate key="repeatDayOfMonth"}</label>
		<input type="radio" {formname key=REPEAT_MONTHLY_TYPE} value="{RepeatMonthlyType::DayOfWeek}" id="{$prefix}repeatMonthWeek"/>
...
<div id ="reserved" class="reserved">
	...
	<input class="reserved" style="width:230px;font-size: 12px;text-align: center" type="text" value="{translate key="Reserved"}" disabled/>
	</br>
	<input class="reserved" style="width:230px;font-size: 12px;text-align: center" type="text" value="" disabled/>
	</br></br>
</div>