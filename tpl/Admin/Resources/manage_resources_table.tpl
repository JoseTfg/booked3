...
<table class="list userList" id="userTable">
	...
	<tr>
		...
		<th>{translate key='ResourceAdministrator'}</th>
		<th>{translate key='UsageConfiguration'}</th>
		<th>{translate key='Permissions'}</th>
		<th>{translate key='Rename'}</th>
...
{*	{assign var=id value=$resource->GetResourceId()}
	...
	*}
	...
	<div style="float:left;max-width:50%;">
		<input type="hidden" class="id" value="{$id}"/>
		<tr>
			<td align="center">{$resource->GetName()|escape}</td>
			
			<td align="center">{if $resource->HasAdminGroup()}
			...
			<td align="center"><a class="update changeConfigurationButton"
															 href="javascript: void(0);">{translate key='ChangeConfiguration'}</a></td>
			<td align="center"><a href="#" class="update changeUsers">{translate key=Users}</a> | <a href="#" class="update changeGroups">{translate key=Groups}</a></td>
			<td align="center"><a class="update renameButton" href="javascript:void(0);">{translate key='Rename'}</a></td>
			<td align="center"><a class="update deleteButton" href="javascript:void(0);">{translate key='Delete'}</a></a></td>
		</tr>
			</div>
	</div>
		{/foreach}
