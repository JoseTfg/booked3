	...
	<form id="reservationForm" method="post" enctype="multipart/form-data">
		...
		<div id="reservationDetails" class="details">
			<ul style="text-align:center;" class="no-style">
				{if $false}
					<button id="promptForChangeUsers" type="button" style="display:inline;">{html_image src="users.png"}</button>
					<div id="changeUserDialog" title="{translate key=ChangeUser}" class="dialog"></div>
				{/if}
				<a style="font-size:14px" id="userName" data-userid="{$UserId}">{$ReservationUserName|truncate:25:"...":true}</a> <input id="userId"
			...
			<ul class="no-style">
				...
				<li style="text-align: center">
					...
					<input style="font-size: 12px;text-align: center;height: 16px;" type="text" id="BeginDate" class="dateinput" value="{formatdate date=$StartDate}"/>
				...
				<li style="text-align: center">
					{translate key='Until'}
					<br/>
					<input style="font-size: 12px;text-align: center;height: 16px;" type="text" id="EndDate" class="dateinput" value="{formatdate date=$EndDate}"/>
				...
				<li class="rsv-box-l" style="text-align: center">
                <textarea style="resize: none;height:40px;width:230px;" id="description" placeholder="{translate key="ReservationDescription"}" name="{FormKeys::DESCRIPTION}" class="input" rows="2" cols="52" tabindex="110">{$Description}</textarea>
				</li>
			</ul>
		</div>
		...
		<div class="reservationButtons" style="position: absolute; left: 56%;">	
		...
		{if $UploadsEnabled}
			{block name='attachments'}{/block}
		{/if}
	</form>

	<div class="hiddenDiv" id="strings">
		<input id="reasonString" type="text" value="{translate key="Reason"}">
		<input id="availableString" type="text" value="{translate key="NotAvailable"}">
		<input id="titleString" type="text" value="{translate key="ReservationTitle"}">
		<input id="descriptionString" type="text" value="{translate key="ReservationDescription"}">
	</div>
